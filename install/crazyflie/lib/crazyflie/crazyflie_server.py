...
class CrazyflieServer(Node):
    def __init__(self):
        ...
        #####
        # Load the crazyflies.yaml file directly
        config_path = os.path.join(get_package_share_directory('crazyflie'), 'config', 'crazyflies.yaml')
        try:
            with open(config_path, 'r') as file:
                self._crazyflies_config = yaml.safe_load(file)
        except Exception as e:
            self.get_logger().error(f"Failed to load crazyflies.yaml: {e}")
            self._crazyflies_config = {}

         # Extract initial positions
        self.initial_positions = {}
        robots_config = self._crazyflies_config.get('robots', {})
        for robot_name, data in robots_config.items():
            # Use 'initial_position' key; default to [0,0,0] if not provided
            self.initial_positions[robot_name] = data.get('initial_position', [0.0, 0.0, 0.0])

        self.get_logger().info(f"Loaded initial positions: {self.initial_positions}")
        #####
    ...
    def _log_pose_data_callback(self, timestamp, data, logconf, uri):
        ...
        self.get_logger().info(f"Received data from drone with name: {cf_name}")
    
        # offset = self._ros_parameters[cf_name]['initial_position']
        offset = self.initial_positions.get(cf_name, [0.0, 0.0, 0.0])


        x = data.get('stateEstimate.x') + offset[0]
        y = data.get('stateEstimate.y') + offset[1]
        z = data.get('stateEstimate.z') + offset[2]
    ...
    def _log_odom_data_callback(self, timestamp, data, logconf, uri):
        ...
        offset = self.initial_positions.get(cf_name, [0.0, 0.0, 0.0])
        # offset = self._ros_parameters[cf_name]['initial_position']
        # offset = self.get_parameter('initial_positions').value.get(cf_name, [0.0, 0.0, 0.0])
